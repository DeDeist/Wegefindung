...
public:
	...
	{
		float dist;										//Distanz zum Startknoten
	...
	void init(int start_n, vector<vector<float>>& dist_matr)										//Initialisierung der Informationsstruktur
	{
		int i;
	...
	int node_select()																				//Methode zur Auswahl des nächsten zu bearbeitenden Knoten
	{
		...
		for (i = 0; i < Nnodes; i++) {																//Schleife über alle Knoten
			if ((n_info[i].dist < min) && (n_info[i].done == 0))									//Wenn die dist kleiner als das Referenz-Minimum ist wird dieser Wert als neue Min. Referenz gesetzt
			{
				min = n_info[i].dist; 
	...
	vector<int> dijkstra(int start_n, int end, vector<vector<float>>& dist_matr, bool median)		//Methode Dijkstra Algorithmus
	{
		...
		for (int n = 0; n < j; n++) {
			...
			init(start_n, dist_matr);																//Informationsstruktur initialisieren
